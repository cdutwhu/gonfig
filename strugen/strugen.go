...
func GenStruct(tomlFile, struName, pkgName, struFile string) bool {
	warnP1OnErrWhen(!sHasSuffix(tomlFile, ".toml"), "%v @tomlFile", fEf("PARAM_INVALID"))
	...
	failP1OnErrWhen(!ucIsUpper(rune(struName[0])), "%v @struName", fEf("PARAM_INVALID"))
	...
	if addPkg {
		...
		if pkgName != "gonfig" {
			struStr += `import "github.com/cdutwhu/gonfig"` + "\n\n"
		} else {
			warner("pkgName should NOT be 'gonfig' unless for self-dev test")
			selfDev = true
		}
	}
...
func GenNewCfg(struFile string) bool {
	...
	src += "\t}\n"
	if !selfDev {
		src += `	return gonfig.InitEnvVar(cfg, mReplExpr, cfgStruName, cfgPaths...)` + "\n"
	} else {
